...
```python
weights = finalmodel.get_layer('title_embedding').get_weights()[0] # get the weights from the embedding layer
vocab = vectorize_layer_title.get_vocabulary()                # get the vocabulary from our data prep for later

# Use PCA to deduct dimension so we can visualiza on 2D surface.
